...
// LIMITATION (library_types.h): 
// The generic APIs are currently (CUDA 10) available for all platforms except Windows. 
// Using these APIs in any other systems will result in compile-time or run-time failures. 
// Their support will be extended in the next releases (CUDA 11). 
