...
class TestFreezing(JitTestCase):
    ...
    def test_freeze_module_with_mutable_tensor(self):
        ...
        # FIXME: deep copy all folded attribute so that m_f has full ownership.
        m_s.a[0] += 5.0
        self.assertFalse(m_f.hasattr('a'))
        out = m_f.forward(torch.tensor([5]))
        expected = [6., 5., 3.]
    ...
    def test_freeze_module_with_aliased_attr(self):
        ...
        m_s.eval()
        m_f = torch._C._freeze_module(m_s._c)
        # if should be assertTrue. scripting is not preserving aliasing (see #33034)
        # once this issue is fixed switch to assertTrue.
        self.assertFalse(m_f.hasattr('a'))
    ...
    # Check attribute a is preserved. Alias analysis detets that 'a' has output writers.
    ...
    def test_freeze_module_with_aliased_attr4(self):
        class FreezeMe(nn.Module):
            def __init__(self):
                super(FreezeMe, self).__init__()
                self.a = [1, 2, 3, 4, 5, 6]
