...
class AttributePropagator {
 ...
 private:
  ...
  bool findConstantAttr(
    ...
    auto it = preservedAttrs_.find(attrModule._ivalue());
    return it == preservedAttrs_.end() ||
        !it->second.count(attrModule.attr(name));
  }
      ...
      for (auto n : block->nodes()) {
        ...
        if (n->kind() == prim::SetAttr || n->kind() == prim::GetAttr) {
          ...
          if (n->kind() == prim::GetAttr) {
            TORCH_CHECK(
                !n->outputs()[0]->type()->cast<InterfaceType>(),
          ...
          if (n->kind() == prim::SetAttr ||
              (!n->outputs()[0]->type()->cast<ClassType>() &&
      ...
      for (auto n : block->nodes()) {
        ...
        if (n->kind() == prim::GetAttr) {
          ...
          if (module_.hasattr(name)) {
            preservedAttrs_[module_._ivalue()].insert(module_.attr(name));
          }
        }
      }
    }
  ...
  void cleanupFrozenModule(std::shared_ptr<Graph>& graph) {
    ...
    for (size_t i = 0; i < N; ++i) {
      ...
      if (it == preservedAttrs_.end() ||
          !it->second.count(module_.attr(name))) {
  ...
  // TODO: handle aliased attribute. e.g. self.b = (self.a, 2) when JIT handles
  // them properly.
  struct CompAttr {
  ...
  std::unordered_map<
      script::ModulePtr,
      std::unordered_set<IValue, HashAttr, CompAttr>>
      preservedAttrs_;
